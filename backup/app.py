...
with tab1:
    ...
    with col_history:
        ...
        if not df_lav.empty:
            exibir = df_lav[['data','cliente','tipo_veiculo','servico','valor','placa']].head(20)
            ...
            st.markdown(f"**Total:** {len(df_lav)} lavagens | **Valor total:** R$ {df_lav['valor'].sum():,.2f}".replace(",","X").replace(".",",").replace("X","."))
        else: st.info("Nenhuma lavagem registrada ainda.")
